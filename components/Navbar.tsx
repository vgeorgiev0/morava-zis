  ...
  return (
    <nav
      className={` flex justify-evenly  sticky w-full top-0 left-0 bg-white rounded-b-full rounded-tl-full transition-all duration-300  drop-shadow-2xl ${
        isOpen ? "h-54 rounded-tl-3xl rounded-bl-full rounded-br-3xl" : ""
      }`}
    >
      <div className="flex flex-row flex-1 gap-10 px-4 md:pl-10">
        <div className="flex flex-1 py-1">
          <Link className=" mt-2" locale={locale} href={"/"} onClick={scroll}>
            <Image
              width={65}
              height={60}
              className="w-16 h-20 md:w-24 md:h-32  flex-1 hover:opacity-80 transition-all duration-500"
              priority={true}
              alt="logo"
              src={logo}
            />
        ...
        <div className="flex flex-2 mr-10 flex-col transition-all duration-500">
          <div
          ...
          <div className="flex flex-1 gap-3 justify-end pt-6 pb-6 ">
            {locales?.map((locale) => (
              <Link
